...
export function SettingsPage({ currentUser, onUserUpdated, onToast }: Props) {
  ...
  return <div className="screen-page"><div className="screen-header"><div><p className="eyebrow">إدارة المتجر</p><h1>الإعدادات</h1><p className="muted">تحكم في إعدادات حسابك والمتجر من مكان واحد.</p></div><button className="outline-button" onClick={load}><Settings2 size={15} /> إعادة تحميل</button></div>
    ...
    <div className="screen-toolbar"><label className="screen-search"><Settings2 size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث في إعدادات المتجر..." /></label><div className="filter-chips"><button className={`filter-chip ${group === 'all' ? 'selected' : ''}`} onClick={() => setGroup('all')}>الكل</button>{groups.map((item) => <button key={item} className={`filter-chip ${group === item ? 'selected' : ''}`} onClick={() => setGroup(item)}>{item}</button>)}</div></div><section className="settings-grid">{loading ? <div className="data-card settings-loading">جار تحميل الإعدادات...</div> : visible.length === 0 ? <div className="data-card empty-state"><Settings2 size={28} /><b>لا توجد إعدادات متاحة</b></div> : visible.map((setting) => { const key = String(setting.key); const type = String(setting.type ?? 'string'); return <div className="setting-card" key={key}><div className="setting-heading"><div><span className="setting-group">{String(setting.group ?? 'store')}</span><h2>{key}</h2></div><span className="type-pill">{type}</span></div><p>{String(setting.description ?? 'إعدادات المتجر')}</p>{type === 'boolean' ? <select value={values[key] ?? ''} onChange={(event) => setValues({ ...values, [key]: event.target.value })}><option value="true">مفعل</option><option value="false">غير مفعل</option></select> : type === 'json' ? <textarea rows={4} value={values[key] ?? ''} onChange={(event) => setValues({ ...values, [key]: event.target.value })} /> : <input type={type === 'integer' || type === 'float' ? 'number' : 'text'} step={type === 'float' ? '0.01' : undefined} value={values[key] ?? ''} onChange={(event) => setValues({ ...values, [key]: event.target.value })} /> }<button className="primary-button save-setting" disabled={saving === key} onClick={() => void save(setting)}>{saving === key ? 'جار الحفظ...' : <><Save size={14} /> حفظ</>}</button></div> })}</section></div>
}
