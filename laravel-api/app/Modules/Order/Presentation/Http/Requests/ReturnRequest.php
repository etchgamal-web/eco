...
final class ReturnRequest extends FormRequest
{
    ...
    {
        return $this->authorizePermission(str_starts_with((string) $this->route()?->getName(), 'customer.returns') ? ($this->isMethod('get') ? 'customer.orders.view' : 'customer.orders.manage') : ($this->route()?->getName() === 'returns.index' ? 'orders.view' : 'orders.return'));
    }
