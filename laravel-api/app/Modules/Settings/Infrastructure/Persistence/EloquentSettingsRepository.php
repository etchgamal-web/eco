...
class EloquentSettingsRepository implements SettingsRepositoryInterface
{
    ...
    {
        $s = Setting::query()->firstOrNew(['key' => $d->key]);
        $s->group = $d->group;
        $s->type = $d->type;
        $s->description = $d->description;
        $s->is_secret = $d->isSecret || (bool) ($s->is_secret ?? false);
        $s->is_encrypted = $d->isEncrypted || (bool) ($s->is_encrypted ?? false);
        if (! ($s->is_secret && $d->value === '********')) {
            $s->setTypedValue($d->value);
        }$s->save();
