...
class EloquentCategoryRepository implements CategoryRepositoryInterface
{
    ...
    {
        return Category::query()->where('slug', $slug)->when($exceptId, fn ($q) => $q->whereKeyNot($exceptId))->exists();
    }

    public function create(CategoryData $data, string $slug): object
    {
        try {
            return Category::query()->create(array_merge($data->toArray(), ['slug' => $slug]))->load('parent');
    ...
    public function update(int $categoryId, CategoryData $data, string $slug): object
    {
        ...
        try {
            $category->update(array_merge($data->toArray(), ['slug' => $slug]));

            return $category->refresh()->load(['parent', 'children'])->loadCount('products');
        } catch (QueryException $e) {
            if ($this->slugExists($slug, $categoryId)) {
                throw new DuplicateSlugException($slug);
            } throw $e;
        }
    }
    ...
    {
        $this->findOrFail($categoryId)->delete();
    }
}
