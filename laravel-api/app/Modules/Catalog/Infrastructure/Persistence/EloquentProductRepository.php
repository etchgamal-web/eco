...
class EloquentProductRepository implements ProductRepositoryInterface
{
    ...
    {
        try {
            return Product::query()->create(array_merge($data->toArray(), ['slug' => $slug]))->load(['brand', 'category']);
    ...
    public function update(int $productId, ProductData $data, string $slug): object
    {
        ...
        try {
            $product->update(array_merge($data->toArray(), ['slug' => $slug]));

            return $product->refresh()->load(['brand', 'category', 'variants.attributeValues.attribute']);
        } catch (QueryException $e) {
    ...
    {
        $this->findOrFail($productId)->delete();
    }

    public function hasVariants(int $productId): bool
    {
        return $this->findOrFail($productId)->variants()->exists();
    }
    ...
    public function createVariant(int $productId, ProductVariantData $data, string $hash, iterable $values): object
    {
        ...
        try {
            ...
            $this->throwVariantConflict($product, $data->sku, $hash);
            throw $e;
        }
    }
    ...
    public function updateVariant(int $variantId, ProductVariantData $data, string $hash, iterable $values): object
    {
        ...
        try {
            ...
            $this->throwVariantConflict($variant->product, $data->sku, $hash, $variant->id);
            throw $e;
        }
    }
    ...
    {
        if ($this->skuExists($sku, $exceptId)) {
            throw new DuplicateSkuException($sku);
        }
        if ($this->combinationExists($product, $hash, $exceptId)) {
