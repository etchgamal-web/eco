...
final class EloquentProductReviewRepository implements ProductReviewRepositoryInterface
{
    public function approvedForProduct(int $productId): iterable
    {
        ...
        return ProductReview::query()->with('user:id,name')->where('product_id', $productId)->where('status', 'approved')->latest()->get();
    }
    ...
    public function summary(int $productId): array
    {
        ...
        return ['count' => (int) $query->count(), 'average_rating' => round((float) ($query->avg('rating') ?? 0), 2)];
    }

    public function createForVerifiedCustomer(int $userId, int $productId, array $data): object
    {
        Product::query()->findOrFail($productId);
        if (ProductReview::query()->where('product_id', $productId)->where('user_id', $userId)->exists()) {
    ...
    {
        return ProductReview::query()->with(['product:id,name', 'user:id,name'])->latest()->get();
    }

    public function moderate(int $reviewId, string $status): object
    {
        if (! in_array($status, ['approved', 'rejected'], true)) {
            throw ReviewNotAllowedException::invalidStatus();
        } $review = ProductReview::query()->findOrFail($reviewId);
        $review->update(['status' => $status]);

        return $review->fresh(['product:id,name', 'user:id,name']);
    }
}
