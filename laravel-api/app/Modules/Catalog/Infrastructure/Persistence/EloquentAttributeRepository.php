...
class EloquentAttributeRepository implements AttributeRepositoryInterface
{
    ...
    {
        return Attribute::query()->create($data->toArray())->load('values');
    }

    public function update(int $attributeId, AttributeData $data): object
    {
        $attribute = $this->findOrFail($attributeId);
    ...
    {
        return $this->findOrFail($attributeId)->values()->whereHas('variants')->exists();
    }

    public function delete(int $attributeId): void
    {
        $this->findOrFail($attributeId)->delete();
    }
}
