...
final class EloquentProductMediaRepository implements ProductMediaRepositoryInterface
{
    public function listForProduct(int $productId): iterable
    {
        ...
        return ProductMedia::query()->where('product_id', $productId)->whereNull('variant_id')->orderBy('sort_order')->orderBy('id')->get();
    }

    public function listForVariant(int $productId, int $variantId): iterable
    {
        $this->variant($productId, $variantId);

        return ProductMedia::query()->where('product_id', $productId)->where('variant_id', $variantId)->orderBy('sort_order')->orderBy('id')->get();
    }
    ...
    {
        $this->removeScoped($productId, null, $mediaId);
    }

    public function removeVariant(int $productId, int $variantId, int $mediaId): void
    {
        $this->variant($productId, $variantId);
        $this->removeScoped($productId, $variantId, $mediaId);
    }
    ...
    public function reorder(int $productId, ?int $variantId, int $mediaId, int $sortOrder): object
    {
        ...
        $media = $query->first();
        if ($media === null) {
            throw new ProductMediaNotFoundException($mediaId);
        }
    ...
    private function variant(int $productId, int $variantId): ProductVariant
    {
        return ProductVariant::query()->where('product_id', $productId)->findOrFail($variantId);
    }
}
