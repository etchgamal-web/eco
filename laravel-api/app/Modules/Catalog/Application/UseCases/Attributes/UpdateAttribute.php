...
final class UpdateAttribute
{
    ...
    public function execute(int $id, AttributeData $d): object
    {
        $this->attributes->findOrFail($id);
        if ($this->attributes->nameExists($d->name, $id)) {
            throw new BusinessRuleException('The attribute name is already in use.');
        }

        return $this->attributes->update($id, $d);
    }
}
