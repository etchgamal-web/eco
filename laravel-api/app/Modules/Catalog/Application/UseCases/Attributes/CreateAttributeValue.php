...
final class CreateAttributeValue
{
    ...
    public function execute(AttributeValueData $d): object
    {
        $this->attributes->findOrFail($d->attributeId);
        if ($this->values->valueExists($d->attributeId, $d->value)) {
            throw new BusinessRuleException('The attribute value already exists.');
        }

        return $this->values->create($d);
    }
}
