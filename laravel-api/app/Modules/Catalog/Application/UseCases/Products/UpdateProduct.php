...
final class UpdateProduct
{
    ...
    public function execute(int $id, ProductData $data): object
    {
        ...
        if (! in_array($data->type, ['simple', 'variable'], true)) {
            throw InvalidProductTypeException::unsupported($data->type);
        }
        if ($data->type === 'simple' && $this->products->hasVariants($id)) {
            throw InvalidProductTypeException::hasVariants();
        }
        $slug = Str::slug($data->slug ?: $data->name);
        if ($this->products->slugExists($slug, $id)) {
