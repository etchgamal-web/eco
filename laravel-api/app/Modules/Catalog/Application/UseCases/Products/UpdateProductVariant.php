...
final class UpdateProductVariant
{
    ...
    public function execute(int $productId, int $variantId, ProductVariantData $data): object
    {
        ...
        if ($product->type !== 'variable') {
            throw InvalidProductTypeException::variantNotAllowed();
        }
        $variant = $this->products->findVariantOrFail($productId, $variantId);
        if ($this->products->skuExists($data->sku, $variant->id)) {
            throw new DuplicateSkuException($data->sku);
        }
        $values = $this->validatedValues($data);
        $hash = $this->hash($values);
        if ($this->products->combinationExists($productId, $hash, $variant->id)) {
            throw InvalidVariantCombinationException::duplicate();
        }
    ...
    {
        $ids = array_values(array_unique($data->attributeValueIds));
        $values = collect($this->attributeValues->findMany($ids));
        if (count($ids) !== count($data->attributeValueIds) || $values->count() !== count($ids) || $values->pluck('attribute_id')->unique()->count() !== $values->count()) {
            throw InvalidVariantCombinationException::invalid();
        }

        return $values;
    }

    private function hash(Collection $values): string
    {
        return hash('sha256', $values->pluck('id')->sort()->implode(':'));
    }
}
