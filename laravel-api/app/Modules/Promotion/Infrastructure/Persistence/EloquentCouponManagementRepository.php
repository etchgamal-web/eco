...
final class EloquentCouponManagementRepository implements CouponManagementRepositoryInterface
{
    ...
    {
        return Coupon::query()->withCount('usages')->latest()->get();
    }

    public function find(int $id): object
    {
        $coupon = Coupon::query()->withCount('usages')->find($id);
        if ($coupon === null) {
...
return $coupon;
    ...
    {
        return Coupon::query()->create(array_merge($data, ['code' => strtoupper($data['code'])]));
    }

    public function update(int $id, array $data): object
    {
        $coupon = $this->find($id);
        $coupon->update(array_merge($data, isset($data['code']) ? ['code' => strtoupper($data['code'])] : []));
