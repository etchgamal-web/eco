...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withSchedule(function (Schedule $schedule): void {
        ...
        try {
            $configured = Setting::query()->where('key', 'cart.abandoned_scan_time')->value('value');
            if (is_string($configured) && preg_match('/^([01]\d|2[0-3]):[0-5]\d$/', $configured)) {
                $time = $configured;
            }$backup = Setting::query()->where('key', 'backup.enabled')->first();
            if ($backup) {
                $backupEnabled = (bool) $backup->getTypedValue();
            }$backupSchedule = Setting::query()->where('key', 'backup.schedule')->value('value');
    ...
    ->withExceptions(function (Exceptions $exceptions): void {
        ...
        $exceptions->render(function (DomainAuthenticationException $e, Request $r) {
            if ($r->is('api/*')) {
        ...
        $exceptions->render(function (PaymentException $e, Request $r) {
            if ($r->is('api/*')) {
                return response()->json(['message' => $e->getMessage()], 422);
            }
        });
        $exceptions->render(function (InvalidWebhookSignatureException $e, Request $r) {
            if ($r->is('api/*')) {
                return response()->json(['message' => $e->getMessage()], 401);
            }
        });
        $exceptions->render(function (SocialConnectionNotFoundException|ConversationNotFoundException $e, Request $r) {
            if ($r->is('api/*')) {
                return response()->json(['message' => $e->getMessage()], 404);
            }
        });
        $exceptions->render(function (SocialCommerceException $e, Request $r) {
            if ($r->is('api/*')) {
                return response()->json(['message' => $e->getMessage()], 422);
            }
        });
        $exceptions->render(function (AiProviderException $e, Request $r) {
            if ($r->is('api/*')) {
                report($e);

                return response()->json(['message' => 'AI service is temporarily unavailable.', 'error_code' => $e->errorCode(), 'retryable' => $e->retryable(), 'correlation_id' => $r->header('X-Correlation-ID')], $e->httpStatus());
            }
        });
        $exceptions->render(function (PaymentNotFoundException|ShippingRateNotFoundException|ShipmentNotFoundException|OrderNotFoundException|ProductNotFoundException|VariantNotFoundException|AttributeNotFoundException|AttributeValueNotFoundException|BrandNotFoundException|CategoryNotFoundException|SettingsNotFoundException|CustomerNotFoundException|CustomerFeatureNotFoundException|AddressNotFoundException|StaffNotFoundException|InventoryNotFoundException $e, Request $r) {
            if ($r->is('api/*')) {
                return response()->json(['message' => $e->getMessage()], 404);
            }
        });
        ...
        $exceptions->render(function (HttpExceptionInterface $e, Request $r) {
            if ($r->is('api/*')) {
                return response()->json(['message' => $e->getStatusCode() === 403 ? 'Forbidden.' : 'Request failed.'], $e->getStatusCode());
            }
        });
