...
final class CustomerFeaturesApiTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->customer->roles()->attach(Role::query()->where('slug', 'customer')->firstOrFail());
        $this->product = Product::query()->create(['name' => 'Phone', 'slug' => 'phone', 'type' => 'simple', 'status' => 'active']);
    }
    ...
    public function test_addresses_default_address_cart_wishlist_preferences_and_notifications(): void
    {
        ...
        $this->assertDatabaseHas('customer_addresses', ['user_id' => $this->customer->id, 'is_default' => 0]);
        $this->actingAs($this->customer)->getJson('/api/v1/customer/addresses/default')->assertOk()->assertJsonPath('data.recipient_name', 'B');
        $secondId = $second->json('data.id');
        $this->actingAs($this->customer)->deleteJson("/api/v1/customer/addresses/{$secondId}")->assertNoContent();
        $this->actingAs($this->customer)->postJson('/api/v1/customer/cart/items', ['product_id' => $this->product->id, 'quantity' => 2])->assertCreated();
        $this->actingAs($this->customer)->getJson('/api/v1/customer/cart')->assertOk()->assertJsonPath('data.items.0.quantity', 2);
        $this->actingAs($this->customer)->postJson('/api/v1/customer/wishlist', ['product_id' => $this->product->id])->assertCreated();
        $this->actingAs($this->customer)->getJson('/api/v1/customer/wishlist')->assertOk()->assertJsonCount(1, 'data');
        $this->actingAs($this->customer)->putJson('/api/v1/customer/preferences', ['data' => ['locale' => 'ar', 'marketing' => false]])->assertOk();
        $this->actingAs($this->customer)->getJson('/api/v1/customer/preferences')->assertOk()->assertJsonPath('data.data.locale', 'ar');
        CustomerNotification::query()->create(['user_id' => $this->customer->id, 'type' => 'order', 'title' => 'Order', 'body' => 'Ready']);
        $this->actingAs($this->customer)->getJson('/api/v1/customer/notifications')->assertOk()->assertJsonCount(1, 'data');
    }
    ...
    public function test_customer_cannot_read_modify_or_delete_another_customers_data(): void
    {
        ...
        $otherAddress =
         CustomerAddress::query()->create(['user_id' => $other->id, 'recipient_name' => 'Other', 'phone' => '012', 'address_line1' => 'Other Street', 'city' => 'Cairo', 'country' => 'EG', 'is_default' => true]);
        CustomerCart::query()->create(['user_id' => $other->id]);
        CustomerWishlist::query()->create(['user_id' => $other->id, 'product_id' => $this->product->id]);
        CustomerPreference::query()->create(['user_id' => $other->id, 'data' => ['secret' => true]]);
    ...
    public function test_customer_can_list_only_own_orders_and_guests_are_rejected(): void
    {
        ...
        $other = User::factory()->create();
        CustomerOrder::query()->create(['user_id' => $other->id, 'status' => 'pending', 'total_amount' => 200]);
